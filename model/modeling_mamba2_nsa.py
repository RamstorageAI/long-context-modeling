...
# from native_sparse_attention.naive import naive_nsa_compression 

from fla.modules import RotaryEmbedding
from model.rotary import apply_rotary_emb_func
...
if is_causal_conv1d_available():
    ...
    raise Exception('causal conv1d not supported')
...
class Mamba2Cache:
    ...
    def __init__(
        ...
        if hasattr(config, 'sw_layers'):
            self.kv_caches = [[None, None] for _ in range(len(config.sw_layers))]

        if hasattr(config, 'nsa_layers'):
            self.nsa_cache = Cache(0)
    ...
    def forward(
        ...
        **kwargs
        ...
        hidden_states = self.mixer(
            hidden_states, cache_params=cache_params, cache_position=cache_position, attention_mask=attention_mask, **kwargs
        )
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids, unsqueeze_dim=1):
    ...
    if len(position_ids.shape) == 2:
        ...
        # print (f"position_ids_flatten: {position_ids_flatten}")
...
class SWAttnLayer(nn.Module):
    def __init__(self, config, layer_idx=-1) -> None:
        super().__init__()
        ...
        self.in_proj = nn.Linear(self.d_model, (self.q_heads + self.kv_heads * 2) * self.head_dim, bias=False)
        self.o_proj = nn.Linear(self.d_model, self.d_model)
        self.sliding_window = config.sliding_window
        self.norm = Mamba2RMSNorm(config.hidden_size, eps=config.layer_norm_epsilon)

        self.enable_rope = getattr(config, 'enable_rope', False)
        if not self.enable_rope:
            self.register_buffer("m", get_alibi_slope(self.q_heads))
        else:
    ...
    def forward(
        ...
        **kwargs
        ...
        q, kv = qkv.split([self.q_heads * self.head_dim, self.kv_heads * 2 * self.head_dim], dim=-1)
        ...
        if not self.enable_rope:
            if cache_params is not None:
                kv_cache = cache_params.kv_caches[self.layer_idx]
            ...
            if cache_params is None:
                ...
                kv_cache = cache_params.kv_caches[self.layer_idx]
                kv_seq_len = kv_cache[0].shape[1] if kv_cache[0] is not None else 0
                # print(f'seq len: {kv_seq_len + q.shape[1]}')
                cos, sin = self.rotary_emb(v, seq_len=cache_position[0] + q.shape[1])
                position_ids = torch.arange(q.shape[1], device=hidden_states.device) + cache_position[0]
                # print(f'q shape: {q.shape}, cos shape: {cos.shape}')
                q, k = apply_rotary_pos_emb(q, k, cos, sin, position_ids)
                if kv_cache[0] is not None and kv_cache[1] is not None:
                    k = torch.cat([kv_cache[0], k], dim=1)
                    v = torch.cat([kv_cache[1], v], dim=1)
                kv_cache[0] = k[:, -self.sliding_window:, :, :]
                kv_cache[1] = v[:, -self.sliding_window:, :, :]
            o = flash_attn_func(
                q, k, v, 
                causal=True, 
                window_size=(self.sliding_window, 0),
        ...
        o = rearrange(o, 'N L h d->N L (h d)')

        return residual + self.o_proj(o)
...
class NSALayer(nn.Module):
    ...
    def __init__(
        ...
        self.num_kv_heads = self.kv_heads = config.num_kv_heads
        ...
        self.block_size = config.chunk_size
        self.block_counts = config.chunk_topk
        # self.window_size = window_size
        self.rope_theta = config.rotary_emb_base
        self.max_position_embeddings = config.max_seq_len
        self.layer_idx = layer_idx
        self.slc_only = getattr(config, 'slc_only', False)
        print(f'slc only: {self.slc_only}')

        self.pre_norm = Mamba2RMSNorm(config.hidden_size, eps=config.layer_norm_epsilon)
    ...
    def forward(
        ...
        **kwargs,
        ...
        residual = hidden_states
        hidden_states = self.pre_norm(residual)
        ...
        # if cache_params is not None:
        #     past_key_values = cache_params.nsa_cache
...
class Mamba2Model(Mamba2PreTrainedModel):
    def __init__(self, config):
        ...
        for idx in range(config.num_hidden_layers):
            if hasattr(config, 'nsa_layers') and (idx + 1) in config.nsa_layers:
                print('add nsa layer')
                layer = NSALayer(config, layer_idx=attn_layer_idx)
                attn_layer_idx += 1
                layers.append(layer)

            if hasattr(config, 'sw_layers') and (idx + 1) in config.sw_layers:
                print('add sw layer')
                layer = SWAttnLayer(config, layer_idx=attn_layer_idx)
                attn_layer_idx += 1
                layers.append(layer)

            layer = Mamba2Block(config, layer_idx=idx)
            layers.append(layer)
...
class Mamba2ForCausalLM(Mamba2PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,  # for now we need this for generation
        ...
        if self.training or not use_cache or self.inference_segment < 0:
            ...
            prev_hidden = None
            hidden_states = None
            for offset in range(0, L, self.inference_segment):
                segment_ids = input_ids[:, offset: offset + self.inference_segment]
                # print(f'segment_ids: {segment_ids.shape}, offset: {offset}')
                segment_mask = None
                if attention_mask is not None:
                    segment_mask=attention_mask[:, offset: offset + self.inference_segment]
                mamba2_outputs = self.backbone(
                    segment_ids,
                    cache_params=cache_params,
                    output_hidden_states=output_hidden_states,
                ...
                cache_params = mamba2_outputs.cache_params
                cache_position[0] += self.inference_segment
