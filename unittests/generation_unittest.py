...
class TestGeneration(TestCase):
    ...
    #     config_path = './configs/ramba_hf/ramba_hf_config_mamba2_unittest.json'
    #     # config_path = './configs/ramba_hf/mamba2_unittest.json'
    ...
    #     while test_times > 0:
    #         test_times -= 1
    #         L = np.random.randint(1, 4 * chunk_size, size=1)[0]
    #         T = max(((L // chunk_size) + 1) * chunk_size, 2 * chunk_size)
    #         print(f'max len: {T}, input len: {L}')
    ...
    #         print(f'is ref score nan: {torch.isnan(ref_scores).any()}, is out scores nan: {torch.isnan(out_scores).any()}')
    #         print(f"Max diff: {(out_scores - ref_scores[:, -(T - L) - 1: -1]).abs().max()}")
    #         self.assertTrue((out_scores - ref_scores[:, -(T - L) - 1: -1]).abs().max() < 0.02)

    def test_gca_new(self):
